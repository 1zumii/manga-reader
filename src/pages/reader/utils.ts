...
export const isBeforeCurrentPage = (
  ...
  if (current.mangaId !== comparative.mangaId) return false;
...
export const getDisplayElements = (containerElement?: Element) => {
  ...
  const displayElements = [...containerElement.children].map((childElement) => {
    try {
  ...
  if (displayElements.includes(undefined)) return undefined;
